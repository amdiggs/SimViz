...
void TOPCon::GPU_Compute_X(float bin_width){
    ...
    float* result = (float*)malloc(tot*sizeof(float));
    ...
    for(int d = 0; d < m_depth; d++){
        for (int r = 0; r < m_rows; r++) {
            for (int c = 0; c < m_cols; c++){
                ...
                float val = (result[r_id] < 2.0f) ? result[r_id] : 2.0f;
...
PinHole::PinHole()
{
    m_vox = (AMD::Voxel*)malloc(25000*sizeof(AMD::Voxel));
}
PinHole::~PinHole()
{
    free(m_vox);
...
void Compute_Ambient(AMD::Vertex_Basic* Verts, float* amb, int num_verts){
    
    
    
    int err;
    int num_el = num_verts;
    cl_vertex* loc_verts = (cl_vertex*)malloc(num_verts*sizeof(cl_vertex));
    
    for(int i = 0; i<num_verts; i++){
        loc_verts[i].pos = Verts[i].pos;
        loc_verts[i].norm = Verts[i].norm;
    }
    
    
    int num_ats = Sim->Num_Atoms();
    Atom* ats = Sim->Atoms();
    sphere* sps = (sphere*)malloc(num_ats*sizeof(sphere));
    float radii[4] = {0.75, 0.45,0.45,0.45};
    AMD::Vec3 Box = 0.5*Sim->Sim_Box();
    for(int i = 0; i < num_ats; i++){
        AMD::Vec3 new_pos = ats[i].Get_Coords() - 0.5*Box;
        sps[i].pos = new_pos;
        int typ = ats[i].Get_Type();
        sps[i].rad = radii[typ - 1];
    }
    std::string kernel_file = "/Users/diggs/Desktop/VolumeData/Resources/Shaders/Comp_Amb.cl";
    std::string KS = Get_Kernel_String(kernel_file);
    const char* source = KS.c_str();
    

   cl_device_id device_id = Get_Device();
    cl_context context = Safe_Get<cl_context>(clCreateContext(0, 1, &device_id, NULL, NULL, &err),&err) ;
    cl_program program = Safe_Get<cl_program>(clCreateProgramWithSource(context, 1, (const char **) &source, NULL, &err), &err);
    CL_Check_Error(clBuildProgram(program, 0, NULL, NULL, NULL, NULL));
    size_t global_dims[3];
    global_dims[0] = (unsigned long)num_el;
    global_dims[1] = 1;
    global_dims[2] = 1;
    size_t out_sz = num_el*sizeof(float);
    //float* AO = (float*)malloc(out_sz);
    
    
    cl_kernel ao_kernel = Safe_Get<cl_kernel>(clCreateKernel(program, "Comp_Ambient_Occlusion", &err),&err);
    
    cl_command_queue commands = Safe_Get<cl_command_queue>(clCreateCommandQueue(context, device_id, 0, &err),&err);
    
    cl_mem sp_mem = Safe_Get<cl_mem>(clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR ,num_ats*sizeof(sphere), sps, &err),&err);
    cl_mem vert_mem = Safe_Get<cl_mem>(clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR ,num_verts*sizeof(cl_vertex), loc_verts, &err),&err);
    cl_mem out_mem = Safe_Get<cl_mem>(clCreateBuffer(context, CL_MEM_WRITE_ONLY | CL_MEM_USE_HOST_PTR ,num_verts*sizeof(float), amb, &err),&err);
    
    
    err = 0;
    CL_Check_Error(clSetKernelArg(ao_kernel, 0, sizeof(cl_mem), &sp_mem));
    CL_Check_Error(clSetKernelArg(ao_kernel, 1, sizeof(int), &num_ats));
    CL_Check_Error(clSetKernelArg(ao_kernel, 2, sizeof(cl_mem), &out_mem));
    CL_Check_Error(clSetKernelArg(ao_kernel, 3, sizeof(cl_mem), &vert_mem));
    CL_Check_Error(clEnqueueNDRangeKernel(commands, ao_kernel, 3, NULL, global_dims, NULL, 0, NULL, NULL));
    clFinish(commands);
    
    
    clReleaseKernel(ao_kernel);
    
    clReleaseMemObject(sp_mem);
    clReleaseMemObject(vert_mem);
