...
{
    Sphere_Basic sp;
    m_VAO.Add_Static_Buffer(sp.verts,sizeof(AMD::Vec3),sp.num_verts());
    m_IBO.Gen_Buffer(sp.indices,sp.num_idx());
    m_clrs_vbo = m_VAO.Add_Dynamic_Instance_Buffer(sizeof(AMD::Vec4));
...
void Atoms_Mesh::Draw(){
    ...
    glBindBuffer(GL_ARRAY_BUFFER,m_clrs_vbo);
    glBufferData(GL_ARRAY_BUFFER, m_num_atoms*sizeof(AMD::Vec4), (void*)m_clrs, GL_DYNAMIC_DRAW);
    ...
    glBufferData(GL_ARRAY_BUFFER, m_num_atoms*sizeof(float), (void*)m_radii, GL_DYNAMIC_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER,0);
...
{
    Arrow ar;
    VertexBuffer vb(ar.verts, ar.num_verts()*sizeof(AMD::Vertex_TX));
...
void Voxel_Mesh::Set_Data(PinHole& ph){
    ...
    for ( int i = 0 ; i < num; i++){
        ...
        m_counts[count] = Data[i].value - 1.0;
...
:m_sh(shader_file), grid_sh(grid_shader_file),m_cut(0.25), m_count(0), vox_len(1.0, 1.0, 1.0)
{
    ...
    m_gs_val_vbo = m_VAO.Add_Dynamic_Buffer(sizeof(float));
    m_max_el = 1000;
    ...
    m_gs_vals = (float*)malloc(m_max_el*sizeof(float));
...
Iso_Mesh::~Iso_Mesh()
{
    ...
    glDeleteBuffers(1, &m_gs_val_vbo);
}
...
void Iso_Mesh::Set_Data(TOPCon &hist) {
    ...
    Iface = Find_Interfaces(hist);
    Iface += AMD::Vec2(-0.5*BOX.z, -0.5*BOX.z);
    Set_Edges();
...
void Iso_Mesh::Draw_Grid() {
    ...
    glBufferData(GL_ARRAY_BUFFER, m_count*sizeof(AMD::Vec4), (void*)m_clrs, GL_DYNAMIC_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER,0);
    glBindBuffer(GL_ARRAY_BUFFER,m_gs_val_vbo);
    glBufferData(GL_ARRAY_BUFFER, m_count*sizeof(float), (void*)m_gs_vals, GL_DYNAMIC_DRAW);
    this->m_VAO.bind();
    glDrawArrays(GL_TRIANGLES,0, m_count);
    this->m_VAO.unbind();
}
...
void Iso_Mesh::Add_Triangle(AMD::Ivec3& tri){
    ...
    for (int i = 0; i<3; i++){
        ...
        tr[i] = p1;
        ave_z += p1.z;
        if(p1.z > Iface.x && p1.z < Iface.y){
            tr_clr = m_clr; //AMD::Vec4(1.0,0.6,0.0,0.6);
            grid_weight = 1.0;
        }
        else{
            tr_clr = m_clr;
        ...
        if(m_count < (m_max_el - 1)){
            m_pos[m_count] = p1;
            m_clrs[m_count] = tr_clr;
            m_gs_vals[m_count] = grid_weight;
            m_count ++;
        }
        else{
            m_max_el +=1000;
            ...
            m_gs_vals = (float*)realloc(m_gs_vals, m_max_el*sizeof(float));
            m_pos[m_count] = p1;
            m_clrs[m_count] = tr_clr;
            m_gs_vals[m_count] = grid_weight;
            m_count ++;
    ...
    ave_z /=3.0;
    if(ave_z > Iface.x && ave_z < Iface.y){
        ph_area += tr.Area();
    }
    return;
}
...
void Wire_Frame::Set_Data(const char* file){
    ...
    float shift = min/(max - min);
    printf("min = %.6f max = %.6f shift = %.4f\n",min,max, shift);
    ...
    for(int i = 0; i < m_h; i++){
        x = -0.5*m_w;
        for(int j = 0; j<m_w; j++){
            val = vals[count]; //(vals[count] - min) / (max - min);
            m_verts[count] = AMD::Vec3(x,y,val);
            count ++;
            x+=1.0;
            if(i == (m_h - 1)){
                if(j < m_w -1){
