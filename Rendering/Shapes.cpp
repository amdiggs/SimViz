...
int Circle::num_idx(){return m_num_idx;}
int Circle::num_verts(){return m_num_verts;}
...
Sphere::Sphere(float e_rad,AMD::Vec3 offset,AMD::Vec4 Color)
:rad(e_rad),m_offset(offset) ,m_clr(Color){
    Gen_points();
    Gen_indices();
    
    AMD::Vec3 Norm_Norm;
    for (int i = 0 ; i< m_num_verts; i++){
        basic_verts[i].pos =  verts[i].pos + m_offset;
        basic_verts[i].clr =  m_clr;
        basic_verts[i].norm = AMD::Normalize(verts[i].norm);
    }
...
void Sphere_Basic::Gen_points(){
    float d_phi = m_dth;
    float theta = PI - m_dth;
    float phi = 0.0;
    float x, y, z;
    int end = ((m_num_theta)/2) -1;
    verts[0] = Vec3(0.0,0.0,-rad);
    m_num_verts ++;
    for ( int i = 0; i<end; i++){
        for (int j = 0; j<m_num_theta; j++){
            phi = j*d_phi;
            ...
            y = rad*sin(phi)*sin(theta);
            z = rad*cos(theta);
            verts[m_num_verts] = Vec3(x,y,z);
            m_num_verts ++;
            }
        theta-=m_dth;
    }
   
    verts[m_num_verts] = Vec3(0.0,0.0,rad);
    m_num_verts ++;
    return;
}


void Sphere_Basic::Gen_indices(){
    int start = 0;
    int off_set = 1;
...
int Sphere_Basic::num_idx() const {return m_num_idx;}
int Sphere_Basic::num_verts() const {return m_num_verts;}
...
void Cone::Gen_points() {
    float dr = (m_rhi - m_rlo)/m_num_z; 
    float r = m_rlo;
    float _z = m_zlo;
    float dz = (m_zhi - m_zlo)/m_num_z;
    float theta = 0.;
    int count = 0;
    float _x, _y;
    ...
    verts[count].pos = Vec3(0.0, 0.0, m_zhi);
    verts[count].clr = Vec4(0.0,0.0,1.0,1.0);
    m_num_verts = count + 1;
}
...
int Cone::num_idx() const {return m_num_idx;}
int Cone::num_verts() const {return m_num_verts;}
...
void Arrow::Gen_indices() {
    ...
    offset = cyl.num_verts();
    for(int i = 0; i<cir.num_idx(); i++){
        indices[count] = cir.indices[i] + offset;
        count ++;
    }
    offset += cir.num_verts();
    for(int i = 0; i<con.num_idx(); i++){
        indices[count] = con.indices[i] + offset;
        count ++;
    }
    m_num_idx = count;
